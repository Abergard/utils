...
function set_prompt_filter()
    ...
    local user = ""
    if(os.execute("net session >nul 2>&1")) then
        user = "*"
    end
    -- build our own prompt
    -- orig: $E[1;32;40m$P$S{git}{hg}$S$_$E[1;30;40m{lamb}$S$E[0m
    -- color codes: "\x1b[1;37;40m"
    local cmder_prompt = "{usr}{env} \x1b[1;32;40m{cwd} {git}{hg} \n\x1b[1;30;40m{lamb} \x1b[0m"
    cmder_prompt = string.gsub(cmder_prompt, "{cwd}", cwd)
    cmder_prompt = string.gsub(cmder_prompt, "{env}", env)
    cmder_prompt = string.gsub(cmder_prompt, "{usr}", user)
    clink.prompt.value = string.gsub(cmder_prompt, "{lamb}", "λ")
end
